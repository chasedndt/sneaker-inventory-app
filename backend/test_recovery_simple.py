#!/usr/bin/env python3
"""
Simple recovery script for orphaned sold items - NO AUTH REQUIRED.
This version uses temporary no-auth endpoints for easy testing.
"""
...
def check_orphaned_items():
    """Check for orphaned sold items using no-auth endpoint"""
    try:
        print("🔍 Checking for orphaned sold items...")
        response = requests.get(f"{BASE_URL}/api/temp-recovery/orphaned-sold-items")
        
        if response.status_code == 200:
            data = response.json()
            print(f"✅ Found {data['orphaned_count']} orphaned items out of {data['total_items']} total items")
            print(f"📊 Active items: {data['active_count']}")
            print(f"📊 Total sales: {data['total_sales']}")
            
            if data['orphaned_items']:
                print("\n🔴 Orphaned Items (marked as sold but no sales record):")
                ...
                if data['active_items']:
                    print("🟢 Active/Listed Items (visible in frontend):")
                    for item in data['active_items']:
...
def restore_orphaned_items(item_ids):
    ...
    try:
        ...
        response = requests.post(
            f"{BASE_URL}/api/temp-recovery/restore-orphaned-items", 
...
def main():
    ...
    print("🚀 Simple Item Recovery (No Auth Required)")
    print("=" * 60)
